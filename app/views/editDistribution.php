...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-group input, .form-group select { width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 0.95rem; }
                    ...
                    <div class="form-group">
                        ...
                        <select id="id_ville" name="id_ville" required>
                            <?php foreach ($villes as $ville): ?>
                                <option value="<?= $ville['id'] ?>" <?= ($ville['id'] == $distribution['id_ville']) ? 'selected' : '' ?>><?= htmlspecialchars($ville['nom']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select id="id_besoin" name="id_besoin" required>
                            <?php foreach ($besoins as $besoin): ?>
                                <option value="<?= $besoin['id'] ?>" <?= ($besoin['id'] == $distribution['id_besoin']) ? 'selected' : '' ?>><?= htmlspecialchars($besoin['nom']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="quantite">Quantité</label>
                        <input type="number" id="quantite" name="quantite" value="<?= htmlspecialchars($distribution['quantite']) ?>" required min="1">
                    </div>
                    <div class="btn-group">
